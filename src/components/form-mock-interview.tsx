...
export const FormMockInterview = ({ initialData }: FormMockInterviewProps) => {
  const form = useForm<FormData>({
    ...
    defaultValues: initialData
      ? {
        ...
        experience: Number(initialData.experience), // ensure it's number
 ...
 const cleanJsonResponse = (responseText: string) => {
  ...
  const generativeAiResponse = async (data: FormData) => {
    const prompt = `
            As an experienced prompt engineer, generate a JSON array containing 5 technical interview questions along with detailed answers based on the following job information. Each object in the array should have the fields "question" and "answer", formatted as follows:Add commentMore actions
      ...
      <div className="mt-4 flex items-center justify-center w-full">
        ...
        {initialData && (
          <Button size={"icon"} variant={"ghost"} >
            <Trash2 className="min-w-4 text-red-500" />
          </Button>
        )}
      </div>
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="w-full space-y-4">
                ...
                <FormControl>
                  <Input
                    disabled={isLoading} className="h-12" placeholder="eg:- Full stack developer" {...field} />
                </FormControl>
              </FormItem>
            )}
          />
            ...
            render={({ field }) => (
              <FormItem className="w-full space-y-4">
                ...
                <FormControl>
                  <Textarea
                    {...field}
                    disabled={isLoading} className="h-12"
                    value={field.value || ""}
                    placeholder="eg:- describe your job description..." />
                </FormControl>
              </FormItem>
            )}
          />
          ...
          <div className="w-full flex items-center justify-end gap-6">
            ...
            >
              {isLoading ? (
                ...
                <Loader className=" animate-spin w-4 h-4 text-black" />
